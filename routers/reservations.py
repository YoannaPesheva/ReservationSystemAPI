...
def create_reservation(reservation: schemas.ReservationCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    if overlapping_reservations:
        raise HTTPException(status_code=400, detail="The requested time slot is already reserved!")
...
def get_reservations(db:Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    if current_user.role == models.UserRole.USER:
        reservations = db.query(models.Reservation).filter(models.Reservation.client_id == current_user.id).all()
    elif current_user.role == models.UserRole.PROVIDER:
        reservations = db.query(models.Reservation).join(models.Hall).filter(models.Hall.provider_id == current_user.id).all()
...
def update_reservation_status(reservation_id: int, new_status: models.ReservationStatus, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    is_provider_owner = current_user.role == models.UserRole.PROVIDER and hall.provider_id == current_user.id

    if not (is_admin or is_provider_owner):
    ...
    elif reservation.status == models.ReservationStatus.CONFIRMED and new_status == models.ReservationStatus.PENDING:
        raise HTTPException(status_code=400, detail="Cannot revert a confirmed reservation back to pending!")
    elif reservation.status == models.ReservationStatus.COMPLETED and new_status != models.ReservationStatus.COMPLETED:
        raise HTTPException(status_code=400, detail="Cannot change the status of a completed reservation!")
